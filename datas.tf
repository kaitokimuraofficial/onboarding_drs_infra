data "aws_iam_policy_document" "drs_deploy_gha_assume" {
  statement {
    ...
    principals {
      type = "Federated"
      identifiers = [
        "arn:aws:iam::${var.kk_account_id}:oidc-provider/token.actions.githubusercontent.com"
      ]
    }
    condition {
      test     = "StringEquals"
...
data "aws_iam_policy_document" "ecs_task_exec" {
  statement {
    ...
    resources = [
      "arn:aws:secretsmanager:${var.aws_region}:${var.kk_account_id}:secret:drs/*"
    ]
  }
...
data "aws_iam_policy_document" "ecs_task_assume" {
  statement {
    ...
    condition {
      ...
      values = [
        "arn:aws:ecs:${var.aws_region}:${var.kk_account_id}:*"
      ]
    }
    condition {
      test     = "StringEquals"
